...
rv.sts.scree <- 
  ...
  ggtitle("Scree plot for non-sparsed\nRV matrix from STATIS") + ggtheme + theme(plot.title = element_text(face = "plain"),  panel.border = element_rect(size = 1, color = "#42376B", fill = NA))
...
### Barplots - fj
sts.cmpfj.dat <- data.frame(res.sts.byraters$mexPosition.Data$Table$Q[,c(1,2)], 
                            flavors = flavors, raters = raters, 
                            ID = rownames(res.sts.byraters$mexPosition.Data$Table$Q), 
