...
rv.speig <- sparseEIGEN(X = distatis.res$res4Cmat$C, k = 5L, init = "svd", rds = rep(0.35*sqrt(I.c), 5))
rownames(rv.speig$vectors) <- rownames(distatis.res$res4Cmat$C)
...
cmp.Smap34 <- createFactorMap(cmp.Splus$f,axis1 = 3, axis = 4,
                        col.background = NULL,
                        col.axes = "#42376B",
                        alpha.axes = 0.5,
                        col.points = wine.dx$ColorCode,
...
# rv.Splus.cor <- rv.S.eig$vector %*% diag(rv.S.eig$values) %*% t(rv.S.eig$vectors)

## reuse the Splus from sparsified RV
I.s <- nrow(rv.Splus)

## eigen decomposition of the compromise
rvcmp.S.eig <- sparseEIGEN(X = rv.Splus, k = 3L, init = "svd", rds = rep(0.6*sqrt(I.s), 3))
