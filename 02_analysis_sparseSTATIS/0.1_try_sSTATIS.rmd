...
sts.cmpfi.plot <- Splus.fi$zeMap_background + Splus.pfi$mapColByBlocks + Splus.fi$zeMap_dots + Splus.fi$zeMap_text + sts.labels + ggtheme
...
## heatmap ??? NOT WORKING!!!
rv.heat <- pheatmap(RVmat, clustering_distance_rows = disRV2, clustering_distance_cols = disRV2, clustering_method = 'ward.D2', 
                    annotation_row = factor(hc.3),
                    annotation_col = factor(hc.3),
                    annotation_colors = hc.col$gc)
...
rv.fi.plot <- rv.fi$zeMap + rv.label

## plot barplot for alpha values
alpha2plot <- res.sts.byraters$mexPosition.Data$InnerProduct$alphaWeights
names(alpha2plot) <- sub(".", "", names(alpha2plot))
rv.alpha <- PrettyBarPlot2(alpha2plot, threshold = 0, color4bar = col.raters$gc[order(alpha2plot)], sortValues = TRUE)
```
...
siplot1 <- ggplot(si.dat, aes(zeroRatio, fitRatio)) + 
  ...
  theme(
    ...
    title = element_text(size = 8, color = c("#7F6CC7", "#FFFFFF", "#FFFFFF")),
  ...
  with(si.dat[si.dat$max == "MAX",], annotate(geom = "point", x = zeroRatio, y = fitRatio, color = "#7F6CC7", size = 3)) +
  ...
  with(si.dat[si.dat$max == "MAX",], annotate(geom = "label", x = zeroRatio + 0.26, y = fitRatio + 0.18, color = "darkorchid4", label = substring(sprintf("%.3f", SI), 2), fill = "#FAFAFA", size = 2.5))
...
siplot2 <- ggplot(si.dat2, aes(zeroRatio, fitRatio)) + 
  ...
  theme(
    ...
    plot.margin = unit(c(0.1,0,0,0), unit = "cm"),
    panel.grid = element_blank()) + 
  with(si.dat2[si.dat2$max == "MAX",], annotate(geom = "point", x = zeroRatio, y = fitRatio, color = "#7F6CC7", size = 3)) +
  ...
  with(si.dat2[si.dat2$max == "MAX",], annotate(geom = "label", x = zeroRatio + 0.26, y = fitRatio + 0.18, color = "darkorchid4", label = substring(sprintf("%.3f", SI), 2), fill = "#FAFAFA", size = 2.5))
...
rv.ssts.scree <- PlotMyScree(eigres.srv, lwd = 0.8, cex = 1.3, title = "Scree plot for the sparsified\nRV space from sSTATIS") + ggtheme
...
sprv.loli <- ggpubr::ggarrange(lol1, lol2, lol3, nrow = 3, common.legend = TRUE)
...
srv.cmp.scree1 <- PlotMyScree(eigres.srv.cmp$subsp1, lwd = 0.8, cex = 1.3) + ggtheme
srv.cmp.scree2 <- PlotMyScree(eigres.srv.cmp$subsp2, lwd = 0.8, cex = 1.3) + ggtheme
srv.cmp.scree3 <- PlotMyScree(eigres.srv.cmp$subsp3, lwd = 0.8, cex = 1.3) + ggtheme

sprv.cmp.scree <- ggpubr::ggarrange(srv.cmp.scree1, srv.cmp.scree2, srv.cmp.scree3, nrow = 3, common.legend = TRUE)
```
...
srv.cmpfi1.plot <- Splus1.fi$zeMap_background + Splus1.pfi$mapColByBlocks + Splus1.fi$zeMap_dots + Splus1.fi$zeMap_text + stsU1.labels + ggtheme
...
srv.cmpfi2.plot <- Splus2.fi$zeMap_background + Splus2.pfi$mapColByBlocks + Splus2.fi$zeMap_dots + Splus2.fi$zeMap_text + stsU2.labels + ggtheme
...
srv.cmpfi3.plot <- Splus3.fi$zeMap_background + Splus3.pfi$mapColByBlocks + Splus3.fi$zeMap_dots + Splus3.fi$zeMap_text + stsU3.labels + ggtheme
...
scmp.scree <- PlotMyScree(eigres.scmp, lwd = 0.8, cex = 1.3, title = "Scree plot for the sparsified compromise\n from sSTATIS") + ggtheme
```
...
ssts.cmpfi.plot <- ssts.fi$zeMap_background + ssts.pfi$mapColByBlocks + ssts.fi$zeMap_dots + ssts.fi$zeMap_text + ssts.labels + ggtheme
